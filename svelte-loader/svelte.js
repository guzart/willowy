...
function getLocator ( source, options ) {
	...
	return function locate$1 ( search, startIndex ) {
...
function locate$1 ( source, search, options ) {
	...
	if ( isClosingTag ) {
		...
		if ( element.children.length ) {
			...
			if ( firstChild.type === 'Text' ) {
				firstChild.data = trimStart$1( firstChild.data );
				if ( !firstChild.data ) element.children.shift();
			}

			if ( lastChild.type === 'Text' ) {
				lastChild.data = trimEnd$1( lastChild.data );
...
function ParseError ( message, template, index ) {
	const { line, column } = locate$1( template, index );
...
function encodeMappings ( original, intro, outro, chunk, hires, sourcemapLocations, sourceIndex, offsets, names ) {
	...
	var locate = getLocator$1( original );
...
function generate ( parsed, source, options ) {
